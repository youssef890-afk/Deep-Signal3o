...
import { Check, CheckCircle2, Eye, EyeOff, Loader2, Lock, Mail, RefreshCw, Signal, User } from 'lucide-react';
import { useAuth } from '@/context/AuthContext';

export default function SignupPage() {
  const { signUp, verifyEmailOtp, resendVerificationCode } = useAuth();
  ...
  const particles = useMemo(() => {
    return Array.from({ length: 35 }, (_, index) => {
      ...
      const colors = ['rgba(255, 60, 140, 0.9)', 'rgba(255, 150, 80, 0.85)', 'rgba(255, 200, 220, 0.8)', 'rgba(120, 255, 190, 0.7)'];
      const color = colors[Math.floor(Math.random() * colors.length)];
      return { id: index, size, color, left: Math.random() * 100, top: Math.random() * 120, duration: Math.random() * 8 + 6, delay: Math.random() * 8 };
    });
  }, []);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!/^[a-z0-9_]{3,30}$/.test(cleanUsername)) {
      setError('Username خاصو يكون بين 3 و30 حرف، ويحتوي غير على a-z و 0-9 و _.');
      return;
    }
    if (password.length < 8) {
      setError('Password خاصو يكون على الأقل 8 حروف.');
      return;
    }
    setLoading(true);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (cleanOtp.length !== 6) {
      setError('دخل رمز التحقق كامل، خاصو يكون 6 أرقام.');
      return;
    }
    setLoading(true);
    const result = await verifyEmailOtp(email, cleanOtp);
    setLoading(false);
    if (result.error) {
      setError(result.error);
      return;
    ...
    setVerified(true);
    setSuccessMessage('تم التأكيد بالبريد الإلكتروني.');
  };
          ...
          <div className="rounded-[28px] border border-pink-400/30 bg-[#140a19] shadow-[0_0_50px_rgba(255,40,120,0.18),0_25px_50px_rgba(0,0,0,0.8)] p-7 text-center">
            <div className="flex justify-center mb-6">
              <div className="relative w-24 h-24 rounded-full bg-emerald-400/10 border border-emerald-400/30 flex items-center justify-center shadow-[0_0_40px_rgba(42,255,154,0.25)]">
                <CheckCircle2 className="w-14 h-14 text-emerald-400" strokeWidth={1.7} />
  ...
  if (verificationMode) {
    return (
      <div className="relative min-h-screen overflow-hidden bg-[#0a0208] text-white flex items-center justify-center px-4" dir="rtl">
          ...
          <div className="rounded-[28px] border border-pink-400/30 bg-[#140a19] shadow-[0_0_50px_rgba(255,40,120,0.18),0_25px_50px_rgba(0,0,0,0.8)] p-6 sm:p-8 text-center">
            <div className="flex justify-center mb-5">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#ff2a78] via-pink-500 to-[#ff8a3d] flex items-center justify-center shadow-[0_0_30px_rgba(255,42,120,0.35)]">
                <Signal className="w-10 h-10 text-white" strokeWidth={2.3} />
              </div>
            </div>
            <h1 className="text-[22px] sm:text-2xl font-bold">تحقق من بريدك الإلكتروني</h1>
            <p className="mt-4 text-sm leading-7 text-white/60">
            ...
            <div className="mt-5 rounded-2xl border border-pink-400/20 bg-[#1a0a15] px-4 py-3">
              <div className="flex items-center justify-center gap-2">
                ...
                <span className="text-sm font-semibold break-all" dir="ltr">{email}</span>
              </div>
            </div>
            <form onSubmit={handleVerifyOtp} className="mt-6">
              <label className="block text-right text-xs text-white/50 mb-2">رمز التحقق</label>
              <input
                type="text"
                inputMode="numeric"
              ...
              {successMessage && (<div className="mt-4 rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-300">{successMessage}</div>)}
              <button
                type="submit"
              ...
              >
                {loading ? (<><Loader2 className="w-5 h-5 animate-spin" />جاري التحقق...</>) : (<><Check className="w-5 h-5" />تأكيد الرمز</>)}
              </button>
            </form>
            <div className="mt-5">
              {countdown > 0 ? (
                <p className="text-xs text-white/40">تقدر تطلب كود جديد بعد <span className="text-pink-400 font-bold">{countdown}s</span></p>
              ) : (
                <button type="button" onClick={handleResend} disabled={resending} className="inline-flex items-center gap-2 text-sm text-pink-400 hover:text-pink-300 disabled:opacity-50">
                  {resending ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                  إرسال رمز جديد
                </button>
              )}
            </div>
            <p className="mt-6 text-xs text-white/35 leading-5">ما لقيتيش الرسالة؟<br />شوف Spam / Junk.</p>
            <button
              type="button"
              onClick={() => { setVerificationMode(false); setOtp(''); setError(null); setSuccessMessage(null); }}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden bg-[#050008] text-white" dir="ltr">
      ...
      <div className="w-full max-w-md animate-slide-up relative z-10">
        <div className="rounded-3xl p-8 shadow-2xl bg-[#140a19] border border-pink-400/20">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-3xl font-bold bg-gradient-to-r from-[#ff2a78] to-[#ff8a3d] bg-clip-text text-transparent">Join Deep Signal</h1>
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            ...
            {error && (<div className="text-sm text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-3">{error}</div>)}
            <button
              type="submit"
              disabled={loading}
